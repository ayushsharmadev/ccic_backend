...
async function seed() {
  try {
    ...
    const BlogCategory = require("../models/BlogCategory.js").default || require("../models/BlogCategory.js");
    const Blog = require("../models/Blog.js").default || require("../models/Blog.js");
    const NewsCategory = require("../models/NewsCategory.js").default || require("../models/NewsCategory.js");
    const News = require("../models/News.js").default || require("../models/News.js");
    const NewsletterSubscription = require("../models/NewsletterSubscription.js").default || require("../models/NewsletterSubscription.js");
    const Notice = require("../models/Notice.js").default || require("../models/Notice.js");
    ...
    const modelsToClean = [
      User, BlogCategory, Blog, NewsCategory, News, NewsletterSubscription, Notice,
    ...
    const user = await User.create({
      username: "admin",
      email: "admin@counsellingguidance.com",
      password: "admin@12345",
      role: "admin",
      firstName: "Admin",
      lastName: "Counsellor",
      phoneNumber: "8789290881",
      isActive: true
    });
    ...
    // 6. Seed additional models (User, Blog, News, Notice, Newsletter)
    console.log("🌱 Seeding additional models (Blogs, News, Notices, Newsletters)...");

    const blogCategory = await BlogCategory.create({
      name: "Medical Guidance",
      slug: "medical-guidance",
      description: "Latest updates, guides and career paths in medical education.",
      color: "#10B981",
      icon: "book",
      displayOrder: 1,
      isActive: true
    });
    console.log("   - Seeded BlogCategory:", blogCategory.name);

    const blog = await Blog.create({
      title: "How to Prepare for NEET UG 2027: A Complete Guide",
      slug: "how-to-prepare-for-neet-ug-2027",
      excerpt: "Confused about how to prepare for NEET UG 2027? Here is a complete guide covering study plans, important chapters, and expert tips.",
      content: "<h2>NEET UG 2027 Preparation Guide</h2><p>Preparation for NEET UG requires a systematic study plan, focus on NCERT books, regular revision, and solving mock tests. Here we break down the monthly roadmap for aspirants...</p>",
      ...
      isFeatured: true,
      author: user._id,
      tags: ["NEET UG", "Preparation Guide", "Medical"],
      status: "published",
      views: 120,
      readTime: 8,
      metaTitle: "How to Prepare for NEET UG 2027 - Complete Guide",
      metaDescription: "Get expert tips, study roadmap, and chapter-wise weightage for NEET UG 2027 preparation.",
      metaKeywords: ["neet 2027", "neet preparation", "medical entry exams"],
      focusKeyword: "neet ug 2027 preparation"
    });
    console.log("   - Seeded Blog:", blog.title);

    const newsCategory = await NewsCategory.create({
      name: "Admission Alerts",
      slug: "admission-alerts",
      description: "Real-time updates about college admissions, seat allocations and schedules.",
      color: "#EF4444",
      displayOrder: 1,
      isActive: true
    });
    console.log("   - Seeded NewsCategory:", newsCategory.name);

    const news = await News.create({
      title: "NMC Releases New Guidelines for MBBS Admissions Abroad",
      slug: "nmc-guidelines-mbbs-admissions-abroad",
      shortDescription: "National Medical Commission (NMC) has published updated regulations for Indian students seeking medical admissions abroad.",
      content: "<h2>NMC Foreign Medical Graduate Regulations</h2><p>The NMC has updated its guidelines emphasizing the requirement of qualifying NEET UG and completing a structured 54-month course duration taught in English medium with clinical training abroad...</p>",
      category: newsCategory._id,
      relatedCountries: [country._id],
      featuredImage: "https://images.unsplash.com/photo-1576091160550-2173dba999ef?w=800",
      isFeatured: true,
      author: user._id,
      tags: ["NMC Guidelines", "MBBS Abroad", "Admissions 2026"],
      status: "published",
      views: 250,
      metaTitle: "NMC New Guidelines for MBBS Abroad Admissions",
      metaDescription: "Stay updated with the latest NMC regulations for Indian students enrolling in foreign medical colleges.",
      metaKeywords: ["nmc guidelines", "mbbs abroad nmc", "foreign medical graduates"],
      focusKeyword: "nmc guidelines mbbs abroad"
    });
    console.log("   - Seeded News:", news.title);

    const newsletterSubscription = await NewsletterSubscription.create({
      email: "subscriber@example.com",
      source: "footer",
      ipAddress: "127.0.0.1",
      userAgent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
      referrer: "https://counsellingguidance.com/"
    });
    console.log("   - Seeded NewsletterSubscription:", newsletterSubscription.email);

    const notice = await Notice.create({
      title: "AIIMS New Delhi Seat Allocation Round 1 Counseling Schedule",
      content: "The online counselling portal for Round 1 seat allocation at AIIMS New Delhi has been opened. Registered candidates must lock their choices before the deadline.",
      link: "https://exams.aiims.edu/counseling",
      displayOrder: 1,
      college: college._id,
      status: "active"
    });

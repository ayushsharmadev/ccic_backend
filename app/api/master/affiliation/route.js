...
// GET /api/master/affiliation - Get all affiliations with pagination and filters
export const GET = withAdminAuth(async (request) => {
  try {
    ...
    // If all parameter is present, return all affiliations without pagination
    if (all) {
      const affiliations = await Affiliation.find({ status: "active" })
        .sort({ name: 1 })
        .lean();
    ...
    // Get affiliations with pagination
...
// POST /api/master/affiliation - Create a new affiliation
export async function POST(request) {
  try {
    ...
    const affiliation = new Affiliation(body);

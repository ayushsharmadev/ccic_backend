  ...
  useEffect(() => {
    ...
    const loadDropdownData = async () => {
      try {
        ...
        console.error("❌ Error loading dropdown data:", error);
        showError("Failed to load dropdown data. Please refresh the page.");
      } finally {
        if (isMounted) {
          setLoading(false);
        }
    ...
    async (e) => {
      ...
      try {
        ...
        const apiData = {
          ...
          hostelFacilities: formData.hostelFacilities || [],
          logo: formData.logo || null,
          banner: formData.banner || null,
        };

        const data = await makeAuthenticatedRequest("/api/colleges", {
          method: "POST",
        ...
        if (data.success) {
          ...
          if (
            ...
            showError("Failed to add college: " + data.error);
          }
        }
      } catch (error) {
        console.error("Error adding college:", error);
        showError("Error adding college. Please try again.");
      } finally {
    ...
    <div className="h-full p-6 bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-600 dark:text-white/70">
          Create a new college entry for CCIC education
        </p>
      </div>
                  ...
                  <div className="relative">
                    ...
                    {formData.haveHospital === "yes" && (
                      <input
                        ...
                        min="0"
                        className="absolute top-0 w-1/2 right-0 px-3 py-2 rounded text-sm outline-none transition-all duration-200 border border-gray-300 dark:border-slate-700 focus:border-primary focus:ring-2 focus:ring-primary-50 bg-white dark:bg-slate-900 text-gray-900 dark:text-white z-10 rounded-l-none"
                      />
                    )}
                  </div>
                </div>

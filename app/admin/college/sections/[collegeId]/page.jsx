...
export default function CollegeSectionsPage({ params }) {
  ...
  const fetchSections = async () => {
    try {
      ...
      const response = await fetch(
        `/api/facility-sections?collegeId=${resolvedParams.collegeId}`,
        {
          headers: {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await fetch(
        `/api/facility-sections/${deleteModal.sectionId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleEdit = (section) => {
    router.push(
      `/admin/college/sections/${resolvedParams.collegeId}/edit/${section._id}`
    );
  };
    ...
    <div className={pageWrapperClass}>
      ...
      <div className="mb-4">
        ...
        <h1 className={headerTitleClass}>
          Sections - {collegeName}
        </h1>
        <p className={headerSubtitleClass}>
          Manage dynamic sections for this college
        </p>
      </div>
      ...
      <div className="flex justify-end mb-4">
        <Link
          href={`/admin/college/sections/${resolvedParams.collegeId}/add`}

...
export default function CollegeFacilityList() {
  ...
  const handleEditFacility = async (facility) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/master/college-facility/${facility.id}`,
        {
          headers: {
  ...
  const handleModalSubmit = async () => {
    try {
      ...
      if (modalType === "add") {
        // Add new facility
        const response = await fetch("/api/master/college-facility", {
          method: "POST",
          headers: {
        ...
        if (data.success) {
          ...
          showSuccess("College facility added successfully!");
        } else {
        ...
        const response = await fetch(
          `/api/master/college-facility/${editingFacility.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        if (data.success) {
          ...
          showSuccess("College facility updated successfully!");
        } else {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await fetch(
        `/api/master/college-facility/${deleteModal.facilityId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const fetchFacilities = async (page = 1, search = "") => {
    try {
      ...
      const response = await fetch(`/api/master/college-facility?${params}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <div className="h-full p-6 bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-600 dark:text-white/70">
          Manage college facilities and their images for CCIC
          institutions
        </p>
      </div>

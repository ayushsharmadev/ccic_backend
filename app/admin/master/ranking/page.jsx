...
export default function RankingList() {
  ...
  const handleAddRanking = () => {
    setModalType("add");
    setFormData({ name: "", rankValue: "", description: "" });
    setShowModal(true);
  };

  const handleEditRanking = async (ranking) => {
    setModalType("edit");
    ...
    try {
      ...
      const response = await fetch(`/api/master/ranking/${ranking.id}`, {
        headers: {
      ...
      if (data.success) {
        setFormData({
          name: data.data.name,
          rankValue: data.data.rankValue,
          description: data.data.description || "",
        });
      } else {
        showError("Failed to fetch ranking data: " + data.error);
        return;
      }
    } catch (error) {
      console.error("Error fetching ranking data:", error);
      showError("Error fetching ranking data");
      return;
    } finally {
  ...
  const handleModalSubmit = async () => {
    try {
      ...
      if (modalType === "add") {
        // Add new ranking
        const response = await fetch("/api/master/ranking", {
          method: "POST",
          headers: {
        ...
        if (data.success) {
          ...
          showSuccess("Ranking added successfully!");
        } else {
          showError("Failed to add ranking: " + data.error);
          return;
        }
      } else {
        // Update existing ranking
        const response = await fetch(
          `/api/master/ranking/${editingRanking.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        if (data.success) {
          ...
          showError("Failed to update ranking: " + data.error);
          return;
        }
      }

      setShowModal(false);
      setFormData({ name: "", rankValue: "" });
      setEditingRanking(null);
    } catch (error) {
      console.error("Error saving ranking:", error);
      showError("Error saving ranking");
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const handleAboutClick = (rankingName, description) => {
    setAboutModal({
      isOpen: true,
      rankingName,
      description: description || "No description available for this ranking.",
    });
  };
  ...
  // Delete handlers
  const handleDeleteClick = (ranking) => {
    setDeleteModal({
      isOpen: true,
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await fetch(
        `/api/master/ranking/${deleteModal.rankingId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (data.success) {
        setRankings((prev) =>
          prev.filter((r) => r.id !== deleteModal.rankingId)
        );
        showSuccess("Ranking deleted successfully!");
      } else {
        showError("Failed to delete ranking: " + data.error);
      }
    } catch (error) {
      console.error("Error deleting ranking:", error);
      showError("Error deleting ranking");
    } finally {
      setDeleteModal({
        isOpen: false,
        rankingId: null,
  ...
  const columns = [
    {
      ...
      width: "minmax(300px, 2fr)", // Minimum width with flexible growth
    },
    {
      key: "rankValue",
      header: "Rank Value",
      ...
      width: "minmax(100px, 1fr)", // Minimum width with flexible growth
    },
    {
      key: "description",
      header: "Description",
  ...
  const fetchRankings = async (page = 1, search = "") => {
    try {
      setLoading(true);
      ...
      const params = new URLSearchParams({
        ...
        search: search,
        status: "active", // Only show active rankings
      });

      const response = await fetch(`/api/master/ranking?${params}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      if (data.success) {
        ...
        const transformedRankings = data.data.map((ranking) => ({
          id: ranking._id,
          name: ranking.name,
  ...
  useEffect(() => {
    ...
    fetchRankings(1, debouncedSearchTerm);
  }, [debouncedSearchTerm]);

  // Page change handler
  const handlePageChange = (page) => {
    setCurrentPage(page); // Update current page immediately
    fetchRankings(page, debouncedSearchTerm);
  };
    ...
    <div className="h-full p-6 bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-600 dark:text-white/70">
          Manage ranking categories and their values for CCIC
          institutions
        </p>
      </div>
      ...
      <ApnaModal
        ...
        title={modalType === "add" ? "Add New Ranking" : "Edit Ranking"}
        onSubmit={handleModalSubmit}
        submitText={modalType === "add" ? "Add Ranking" : "Update Ranking"}
        cancelText="Cancel"
        size="md"
          ...
          <div>
            ...
            {formLoading ? (
              ...
              <input
                ...
                placeholder="Enter ranking name (e.g., National Institutional Ranking Framework, Business Today, ARIIA)"
                required
          ...
          <div>
            ...
            {formLoading ? (
              ...
              <input
                ...
                onChange={handleInputChange}
                placeholder="Enter rank value (e.g., 25, 76, 100, 45)"
                required
                min="1"
                className="w-full px-3 py-2 border border-gray-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-sm bg-white dark:bg-slate-900/70 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-white/50 transition-colors"
              />
          ...
          <div>
            ...
            {formLoading ? (
              ...
              <textarea
                ...
                onChange={handleInputChange}
                placeholder="Enter ranking description (optional)"
                rows={3}
      ...
      <ApnaModalConfirmation
        ...
        message={`Are you sure you want to delete "${deleteModal.rankingName}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
      ...
      <ApnaModal
        ...
        title={`${aboutModal.rankingName} - Description`}
        size="md"
        showFooter={false}
      >
        <div className="p-4">

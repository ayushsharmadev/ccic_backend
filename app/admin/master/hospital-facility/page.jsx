...
export default function HospitalFacilityList() {
  ...
  const handleEditFacility = async (facility) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/master/hospital-facility/${facility.id}`,
        {
          headers: {
  ...
  const handleModalSubmit = async () => {
    try {
      ...
      if (modalType === "add") {
        // Add new facility
        const response = await fetch("/api/master/hospital-facility", {
          method: "POST",
          headers: {
        ...
        const response = await fetch(
          `/api/master/hospital-facility/${editingFacility.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        if (data.success) {
          ...
          showSuccess("Hospital facility updated successfully!");
        } else {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await fetch(
        `/api/master/hospital-facility/${deleteModal.facilityId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const fetchFacilities = async (page = 1, search = "") => {
    try {
      ...
      const response = await fetch(`/api/master/hospital-facility?${params}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <div className="h-full p-6 bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-600 dark:text-white/70">
          Manage hospital facilities and their images for CCIC
          institutions
        </p>
      </div>
          ...
          <div>
            ...
            {formLoading ? (
              ...
              <input
                ...
                placeholder="Enter facility name (e.g., Emergency Ward, ICU, Operation Theater)"
                required

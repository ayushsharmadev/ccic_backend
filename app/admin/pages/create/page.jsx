...
const CreatePage = () => {
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === "title") {
      ...
      const canonicalUrl = autoSlug
        ? `${window.location.origin}/page/${autoSlug}`
        : "";
  ...
  const handleSaveSection = () => {
    ...
    if (editingSectionIndex !== null) {
      ...
      updatedSections[editingSectionIndex] = {
        ...
        id: formData.sections[editingSectionIndex].id || currentSection.id,
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      const response = await apiClient.post("/api/pages", formData);
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className={headerSubtitleClass}>
              Create a new dynamic page for CCIC website
            </p>
          </div>
          <button
            type="button"

...
export default function EditDistanceMeter() {
  ...
  // Fetch distance meter data
  useEffect(() => {
    const fetchDistanceMeter = async () => {
      try {
        setLoading(true);
        const token = getAccessToken();

        const response = await fetch(`/api/distance-meters/${params.id}`, {
          headers: {
        ...
        if (data.success) {
          const distanceMeter = data.data;
          setFormData({
            name: distanceMeter.name || "",
            shortDescription: distanceMeter.shortDescription || "",
            icon: distanceMeter.icon || null,
            status: distanceMeter.status || "active",
          });
        } else {
          showError("Failed to fetch distance meter data");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        showSuccess("Distance meter updated successfully!");
        router.push("/admin/distance-meters");
      } else {
        showError(data.error || "Failed to update distance meter");
      }
    } catch (error) {
      console.error("Error updating distance meter:", error);
      showError("Failed to update distance meter. Please try again.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="h-full p-4 sm:p-5 md:p-6 bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {saving ? "Updating..." : "Update Distance Meter"}
          </button>
        </div>
      </form>
    </div>

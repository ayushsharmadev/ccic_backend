    ...
    const fetchCountryData = async () => {
      try {
        ...
        if (result.success) {
          ...
          if (countryData.logo) {
            setLogoPreview(countryData.logo);
          }
        } else {
          showError(result.error || "Failed to fetch country data");
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.logo) {
        if (typeof formData.logo === "string") {
          ...
          try {
            ...
            logoFormData.append("type", "countries");
            logoFormData.append("identifier", "country-logo");
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          router.push("/admin/location/country");
        }, 2000);
      } else {
        showError(result.error || "Failed to update country");
      }
    } catch (error) {

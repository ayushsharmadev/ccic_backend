...
export default function EditDistrictPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const districtResponse = await fetch(
          `/api/locations/districts/${params.id}`,
          {
        ...
        if (districtResult.success) {
          ...
          setFormData({
            ...
            country:
              districtData.state?.country?._id ||
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/locations/districts/${params.id}`, {
        method: "PUT",
        headers: {
      ...
      if (result.success) {
        ...
        showError(result.error || "Failed to update district");
      }
    } catch (error) {
      ...
      <div className="bg-white dark:bg-slate-900/70 border border-gray-200 dark:border-slate-800 rounded-lg p-5 shadow-sm transition-colors duration-300">
        <form onSubmit={handleSubmit}>
          ...
          <div className="border-t border-gray-200 dark:border-slate-800 pt-4 flex justify-end gap-2">
            ...
            >
              {submitting ? "Updating..." : "Update District"}
            </button>
          </div>
        </form>
      </div>
